...
export default class PlayerV2 extends React.Component {
  ...
  componentWillUnmount() {
    ...
    let {chapterInfo, uuid} = this.props;
    // AsyncStorage.getItem('token', (err, result) => {
    //   console.log('token--------', result, uuid)
    //   if (result && result.indexOf('_') !== -1) {
    //     let arr = result.split('_');
    //     let userId = arr[0];
    //     post(this, 'api/sendHyDataPost', {eventName: 'stopPlayVideo', userId: userId,corId: chapterInfo.corId, chapterId: chapterInfo.videoId,activeId: uuid}, (res) => {}, () => {})
    //   }
    // })
  ...
  play() {
    this.setState({
      ...
      //       post(this, 'api/sendHyDataPost', {eventName: 'playVideo', userId: userId,corId: chapterInfo.corId, chapterId: chapterInfo.videoId,activeId: uuid}, (res) => {
      //         if (res.code === 1) {
      //           this.setState({
      //             isFirstClick: false
      //           })
      //         }
      //       }, () => {})
      //     } else if (!this.state.isFirstClick && this.state.paused) {
      //       post(this, 'api/sendHyDataPost', {eventName: 'stopPlayVideo', userId: userId,corId: chapterInfo.corId, chapterId: chapterInfo.videoId,activeId: uuid}, (res) => {}, () => {})
      //     }
      //   }
      // })
  ...
  render() {
    ...
    return (
      <View style={fullScreen ? this.formatFullScreenContainer() : containerStyle}
            activeOpacity={1}>
        <Video source={{uri: url}} //require('../img/player/English_sing.mp3')
...

        {
          paused ?
            <TouchableOpacity activeOpacity={0.8}
              ...
              <Image style={{width: rpx(75), height: rpx(75)}} source={require("./../img/player/play_video_icon.png")}/>
          ...
          <View style={[styles.verticalSliderContainer, {position: 'absolute', top: rpx(120), left: 0, right: 0}]}>
            <View style={styles.verticalSlider}>
              <Image style={{width: rpx(32), height: rpx(32)}}
                     source={volume <= 0 ? require('../img/player/no_volume.png') : require('../img/player/volume.png')}/>
...

        {
          ...
          <View style={[styles.verticalSliderContainer, {position: 'absolute', top: rpx(120), left: 0, right: 0}]}>
            <View style={styles.verticalSlider}>
              <Image style={{width: rpx(32), height: rpx(32)}}
                     source={require('../img/player/brightness.png')}/>
              <View style={{height: rpx(4), width: rpx(192), flexDirection: 'row'}}>
        ...
        }
        {
          ...
          <View style={styles.loadingContainer}>
            <Image style={{width: rpx(100), height: rpx(100)}} source={require('../img/player/loading.gif')}/>
          </View>
        }
        {
          isTouchedScreen ?
                  ...
                  fullScreen ?
                    <TouchableOpacity
                      ...
                      <Image style={{width: rpx(22), height: rpx(40)}}
                             source={require('../img/player/back_arrow_icon_white.png')}/>
                    </TouchableOpacity> : null
                }
                {
                  fullScreen ?
            ...
            <View style={[styles.toolBarStyleInner, {width: w}]}>
              <TouchableOpacity activeOpacity={0.8} onPress={() => this.play()}>
                <Image style={{width: rpx(50), height: rpx(50)}}
                       source={this.state.paused ? require('./../img/player/play.png') : require('./../img/player/pause.png')}/>
              </TouchableOpacity>
              <View style={styles.progressStyle}>
                ...
                <Slider
                  ...
                  thumbImage={require('../img/player/slider.png')}
                  step={1}
              ...
              </View>
              {
                fullScreen ?
                  <TouchableOpacity activeOpacity={0.8} onPress={() => {
                    ...
                    <Image style={{width: rpx(50), height: rpx(50)}} source={require("./../img/player/not_full_screen.png")}/>
                  </TouchableOpacity> :
                  <TouchableOpacity activeOpacity={0.8} onPress={() => {
                    this.toggleFullScreen()
                  }}>
                    <Image style={{width: rpx(50), height: rpx(50)}} source={require("./../img/player/full_screen.png")}/>
                  </TouchableOpacity>
              }
            </View>
          </View>
